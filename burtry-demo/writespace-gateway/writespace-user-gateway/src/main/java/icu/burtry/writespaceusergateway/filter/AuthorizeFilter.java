package icu.burtry.writespaceusergateway.filter;

import com.fasterxml.jackson.core.JsonProcessingException;
import com.fasterxml.jackson.databind.ObjectMapper;
import icu.burtry.writespaceusergateway.result.Result;
import icu.burtry.writespaceusergateway.utils.JwtUtil;
...
public class AuthorizeFilter implements GlobalFilter {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        // 放行登录和验证码相关的路径
        if (request.getURI().getPath().contains("/login") || request.getURI().getPath().contains("/code") || request.getURI().getPath().contains("/register")) {
